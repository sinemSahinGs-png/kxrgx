...
uniform float uPerFrameIntensity;
...
void main() {
  ...
  if (uIsMoving) {
    ...
    if (len > 0.001) {
      ...
      float intensity = smoothstep(lineWidth, 0.0, dist) * uPerFrameIntensity;
      newValue += intensity;
    }
  }
...
void main() {
  ...
  float threshold = uRevealThreshold;
  float edgeWidth = uEdgeWidthBase / max(uDpr, 1.0);
  float t = smoothstep(threshold, threshold + edgeWidth, fluid);

  float halo = smoothstep(0.0, threshold * uHaloUpperMul, fluid) * (1.0 - t);
  vec3 tintedTop = mix(topColor.rgb, uHaloGray, halo * uHaloMixStrength);
